...
def build_scoped_query_filters(
    ...
    for key, value in (user_filters or {}).items():
        ...
        if not normalized_key or _normalized_filter_key(normalized_key) in _RESERVED_FILTER_KEYS:
            continue
        if not isinstance(value, (str, int, float, bool)):
            raise ValueError(f"Unsupported query filter value for '{normalized_key}'.")
        if isinstance(value, float) and not math.isfinite(value):
            raise ValueError(f"Query filter value for '{normalized_key}' must be finite.")
