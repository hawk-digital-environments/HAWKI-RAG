...
def _apply_collection_scope(qdrant: QdrantDeletionStore, collection: str | None) -> None:
    if not collection:
        return
...
def _neo4j_delete_payload(result: object) -> _Neo4jDeletePayload:
    ...
    return {
        "relationships_deleted": _count_or_zero(result_mapping.get("relationships_deleted")),
