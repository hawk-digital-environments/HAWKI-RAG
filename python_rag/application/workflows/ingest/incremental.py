...
from application.workflows.ingest.page_registry import IngestedPageRecord, build_page_registry_record
from infrastructure.vectorstore.payloads import build_match_filter

PAGE_URL_KEYS = ("canonical_url", "page_url", "original_url", "url")
URL_LOOKUP_KEYS = ("canonical_url", "page_url", "original_url", "url", "source_url")
PATH_IDENTITY_KEYS = ("relative_path", "file_path", "converted_path", "storage_path", "original_path")
...
class IncrementalIngestPlan:
    ...
    unchanged_page_records: list[IngestedPageRecord] = field(default_factory=list)
    unchanged_chunks: int = 0


def stable_document_id_from_payload(payload: Mapping[str, Any], fallback_doc_id: str) -> tuple[str, str | None]:
    ...
    digest = hashlib.sha256(identity_key.encode("utf-8", errors="ignore")).hexdigest()[:40]
...
def _find_existing_payload(qdrant: Any, doc_id: str, payload: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    for key in URL_LOOKUP_KEYS:
        ...
        for value in values:
            if key == "source_url" and relative_identity:
                _append_filter(filters, seen, {key: value, "relative_path": relative_identity})
                continue
