...
def _build_test_client(tmp_path: Path, dependencies: Any) -> TestClient:
...
class TestQueryApiFlow:
    ...
    def test_valid_request_is_validated_and_delegated_with_a_stable_response(
        ...
        with patch(
            ...
            with _build_test_client(tmp_path, dependencies) as client:
    ...
    def test_missing_authorized_scope_is_rejected_before_delegation(
        ...
        with patch(
            ...
            with _build_test_client(tmp_path, _dependencies()) as client:
                response = client.post(
                    "/query",
