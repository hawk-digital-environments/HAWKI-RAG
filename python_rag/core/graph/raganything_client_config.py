...
from core.graph.raganything_utils import graph_embed_junk_reason, junk_embedding_sentinel, normalize_graph_embed_text
from core.graph.raganything_settings import parse_optional_int
...
def _build_llm_model_func(graph_provider: Any, settings: RagAnythingGraphSettings, *, logger_obj: logging.Logger) -> Any:
    ...
    async def embed_many(texts: Any) -> Any:
        ...
        for idx, raw in enumerate(text_list):
            ...
            if reason is not None:
                filtered += 1
                out_vectors[idx] = junk_embedding_sentinel(text_norm or str(raw or ""), embed_dim)
        ...
        for idx, value in enumerate(out_vectors):
            if value is None:
                out_vectors[idx] = junk_embedding_sentinel(str(text_list[idx] or ""), embed_dim)
    ...
    *,
    ...
    try:
        ...
        if neo4j_env_applied:
            logger_obj.info("LightRAG Neo4j env prepared: %s", {k: v for k, v in neo4j_env_applied.items()})
