...
def load_raganything_graph_settings() -> RagAnythingGraphSettings:
    ...
    return RagAnythingGraphSettings(
        ...
        graph_embed_junk_allowlist=os.environ.get("GRAPH_EMBED_JUNK_ALLOWLIST", "").strip(),
        graph_embed_junk_denylist=os.environ.get("GRAPH_EMBED_JUNK_DENYLIST", "").strip(),
        graph_temperature=os.environ.get("GRAPH_TEMPERATURE", "").strip(),
        ollama_chat_timeout=os.environ.get("OLLAMA_CHAT_TIMEOUT", "").strip(),
        ollama_embed_nan_zero_fallback=_bool_env("OLLAMA_EMBED_NAN_ZERO_FALLBACK", True),
