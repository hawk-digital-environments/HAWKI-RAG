...
def _requests_module() -> Any:
    return import_required_module(
        "requests",
        install_hint="Install python_rag/requirements.txt to use the LiteLLM provider.",
    )
...
def _temperature(value: float | None, default: float) -> float:
    ...
    try:
        ...
        raise ValueError("LiteLLM temperature must be a number between 0 and 2.") from exc
...
def _normalize_content_part(part: Any) -> dict[str, Any]:
    ...
    if part_type in {"image_url", "input_image"}:
        image_value = part.get("image_url") or part.get("image") or part.get("image_data")
        ...
        if image_url is None:
            raise ValueError("LiteLLM image content must include a non-empty URL or base64 value.")
...
class LiteLLMProvider:
    ...
    def __init__(self) -> None:
        ...
        self.embed_model = _required_setting("LITELLM_EMBED_MODEL", _DEFAULT_EMBED_MODEL)
        self.vision_model = _required_setting("LITELLM_VISION_MODEL", _DEFAULT_VISION_MODEL)
    ...
    def _configured_temperature() -> float:
        ...
        try:
            ...
            raise LiteLLMConfigurationError(str(exc).replace("LiteLLM temperature", "LITELLM_TEMPERATURE")) from exc
    ...
    def embed(self, text: str) -> list[float]:
        ...
        for value in embedding:
            if isinstance(value, bool):
                raise RuntimeError("LiteLLM embeddings returned a non-numeric vector value.")
            try:
            ...
            if not math.isfinite(numeric_value):
                raise RuntimeError("LiteLLM embeddings returned a non-finite vector value.")
    ...
    def chat(
        ...
        *,
        ...
        payload = {
            ...
            "messages": [{"role": "system", "content": str(system or "")}] + normalized_messages,
    ...
    def vision_chat(
        ...
        *,
        ...
        if not any(message["role"] == "system" for message in normalized_messages):
            normalized_messages.insert(0, {"role": "system", "content": str(system or "")})
        ...
        content = _chat_content(response)
        if content is None:
            raise RuntimeError("LiteLLM vision chat returned an unexpected or empty response.")
        return content
    ...
    def _post_json(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(f"LiteLLM {operation} request timed out: {detail}") from exc
