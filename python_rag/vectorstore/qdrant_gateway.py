...
class QdrantHTTPGateway:
    ...
    def upsert(self, points: list[dict[str, Any]], *, timeout: float) -> Response:
        return self.send(build_upsert_points_request(self.collection, points, timeout=timeout))
    ...
    def delete_by_filter(self, filter_body: Dict[str, Any], *, timeout: float) -> Response:
        return self.send(
            build_delete_by_filter_request(
                self.collection,
