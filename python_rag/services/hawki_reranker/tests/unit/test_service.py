...
def test_health_does_not_invoke_the_model(
    ...
    response = asgi_test_client_class(app).get("/health")
...
def test_rerank_preserves_document_indices_and_applies_top_n(
    asgi_test_client_class: type[Any],
) -> None:
    model = FakeModel("test-model")
    app = create_app(
    ...
    response = asgi_test_client_class(app).post(
        "/v1/rerank",
        json={
            "query": "authorization",
...
def test_whitespace_only_content_returns_a_client_error(
    asgi_test_client_class: type[Any],
) -> None:
    app = create_app(
        settings=RerankerSettings(model_name="test-model"),
