...
def build_graph_runtime_summary(
    *,
    ...
    return {
        ...
        "doc_status_storage": runtime_meta.get(
            "doc_status_storage", "JsonDocStatusStorage"
        ),
        "graph_storage": runtime_meta.get("graph_storage", "NetworkXStorage(default)"),
        "neo4j": {
            "uri": str(settings.neo4j_uri).strip()
            or str(settings.neo4j_bolt_url).strip()
            or "",
            "database": str(settings.neo4j_database or "").strip() or "neo4j (default)",
            "user": str(settings.neo4j_username).strip()
            or str(settings.neo4j_user).strip()
            or "",
        },
        "doc_status_chunks": {
        ...
        "resilience": {
            ...
            "graph_embed_junk_denylist_configured": bool(
                settings.graph_embed_junk_denylist.strip()
            ),
            "graph_embed_junk_allowlist_configured": bool(
                settings.graph_embed_junk_allowlist.strip()
            ),
        },
    }
