...
def annotate_duplicate_skip_metadata(
    ...
    *,
    ...
    if not is_duplicate_doc_record(
        doc_id, doc, failed_status_value=failed_status_value
    ):
        return doc
    if not isinstance(doc, dict):
        return doc
...
def count_status_records(
    ...
    *,
    ...
    for doc_id, doc in records:
        if is_duplicate_doc_record(
            str(doc_id), doc, failed_status_value=failed_status_value
        ):
            counts[duplicate_count_key] += 1
            continue
        status_val = (
            str((doc or {}).get("status") or "") if isinstance(doc, dict) else ""
        )
