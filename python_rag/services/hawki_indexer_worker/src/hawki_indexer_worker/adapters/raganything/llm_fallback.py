...
def extract_triplets_from_text_with_provider(
    ...
    *,
    ...
    try:
        ...
        log.warning(
            "graph:extract_triplets llm-fallback failed doc_id=%s error=%s",
            doc_id or "-",
            exc,
        )
        return []
    ...
    if triplets:
        log.info(
            "graph:extract_triplets llm-fallback doc_id=%s triplets=%s",
            doc_id or "-",
            len(triplets),
        )
    return triplets
...
def parse_llm_triplet_response(
    response: str, *, max_triplets: int = _MAX_TRIPLETS
) -> list[Triplet]:
...
def _user_message(
    ...
    *,
    ...
    image_note = ""
    paths = [
        path for path in image_paths or [] if isinstance(path, str) and path.strip()
    ]
    if paths:
