...
def scrub_raganything_kv_graph_junk(
    *,
    ...
    if full_entities:
        target_keys = (
            list(full_entities.keys())
            if full_scan
            else ([rag_doc_id] if rag_doc_id in full_entities else [])
        )
    ...
    if full_relations:
        target_keys = (
            list(full_relations.keys())
            if full_scan
            else ([rag_doc_id] if rag_doc_id in full_relations else [])
        )
        for key in target_keys:
    ...
    if entity_chunks:
        if full_scan:
            keys_to_drop = [
                k for k in list(entity_chunks.keys()) if is_junk_graph_label(k)
            ]
        else:
