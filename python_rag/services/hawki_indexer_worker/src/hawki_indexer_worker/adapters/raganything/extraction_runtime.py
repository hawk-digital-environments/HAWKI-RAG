...
from hawki_indexer_worker.adapters.raganything.runtime import clear_lightrag_temp_graph
from hawki_indexer_worker.adapters.raganything.utilities import dedupe_triplets
        ...
        out.append(
            {
                ...
                "image_caption": [
                    "Original uploaded image associated with this converted OCR text."
                ],
                "image_footnote": [],
...
def stable_raganything_doc_id(
    doc_id: str | None, file_path: str | None, content_list: list[dict[str, Any]]
) -> str:
    content_text = "\n".join(
        str(item.get("text") or "") for item in content_list if isinstance(item, dict)
    )
    digest = hashlib.sha1(content_text.encode("utf-8", errors="ignore")).hexdigest()[
        :16
    ]
...
def raganything_file_ref(doc_id: str | None, file_path: str | None) -> str:
    ...
    safe_doc_id = (
        re.sub(r"[^A-Za-z0-9_.-]+", "_", str(doc_id or "graph_doc")).strip("._-")
        or "graph_doc"
    )
    ...
    *,
    ...
    async def _insert_and_export() -> list[tuple[str, str, str]]:
        ...
        try:
            ...
            log.warning(
                "RAG-Anything graph insert failed request_id=%s: %s",
                request_id or "-",
                exc,
            )
            return []
        ...
        if graph_obj is None:
            log.warning(
                "RAG-Anything graph storage is unavailable after insert request_id=%s.",
                request_id or "-",
            )
            _clear_temp_graph(
                neo4j_database,
        ...
        try:
            ...
            log.warning(
                "RAG-Anything graph edge export failed request_id=%s: %s",
                request_id or "-",
                exc,
            )
            _clear_temp_graph(
        ...
        if not isinstance(edges, list):
            log.warning(
                "RAG-Anything graph edge export returned unexpected type=%s request_id=%s",
                type(edges).__name__,
                request_id or "-",
            )
            return []
