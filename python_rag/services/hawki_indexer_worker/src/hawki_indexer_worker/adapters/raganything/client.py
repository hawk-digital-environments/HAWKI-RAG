...
from hawki_indexer_worker.adapters.raganything.cache import (
    scrub_raganything_kv_graph_junk as _scrub_raganything_kv_graph_junk,
)
from hawki_indexer_worker.adapters.raganything.graph_loop import RagAnythingGraphLoop
...
class RagAnythingGraphService:
    ...
    def graph_cache_fingerprint(
        self, provider: object, *, neo4j_database: str | None = None
    ) -> str:
    ...
    def triplets_from_edges(
        self, *, edges: list[dict[str, Any]], file_ref: str, created_at_floor: int
    ...
    def _init_client(
        self, provider: Any, *, neo4j_database: str | None = None
    ) -> Optional[Any]:
    ...
    def get_or_create_client(
        ...
        cache_key = self.graph_cache_fingerprint(
            provider, neo4j_database=neo4j_database
        )
        with self._rag_graph_lock:
    ...
    def extract_triplets(
        ...
        *,
        ...
        if not content_list:
            logger.info(
                "graph:extract_triplets skipping empty/tiny content doc_id=%s",
                doc_id or "-",
            )
            return []
        ...
        if self._settings.graph_reset_cache_per_doc:
            ...
            if cleared.get("failed"):
                logger.warning(
                    "RAG-Anything graph cache reset had failures: %s",
                    cleared.get("failed"),
                )
            else:
                logger.info(
        ...
        if client is None:
            logger.warning(
                "RAG-Anything graph client is not initialized; returning 0 triplets."
            )
            return []
