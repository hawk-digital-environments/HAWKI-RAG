...
def build_triplets_by_doc(
    ...
    *,
    ...
    for doc_id, parts in grouped.items():
        ...
        triplets, error = run_graph_extract_with_timeout(
            _extract, graph_doc_timeout_s, allow_alarm=use_alarm
        )
        ...
        if error:
            ...
            logger.warning(
                "graph:extract request_id=%s doc=%s failed=%s ms=%.2f",
                request_id or "-",
                doc_id,
                error,
                extract_ms,
            ...
            logger.info(
                "graph:extract request_id=%s doc=%s triplets=%s ms=%.2f",
                request_id or "-",
                doc_id,
                len(triplets),
                extract_ms,
            ...
            if graph is not None and triplets:
                ...
                logger.info(
                    "graph:neo4j upsert request_id=%s doc=%s triplets=%s",
                    request_id or "-",
                    doc_id,
                    len(triplets),
                )
