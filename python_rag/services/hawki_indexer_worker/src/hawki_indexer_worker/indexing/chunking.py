...
from hawki_indexer_worker.indexing.incremental import (
    content_hash_for_text,
    normalized_page_url,
    stable_document_id_from_payload,
)
from hawki_indexer_worker.indexing.models import IngestChunkRecord, IngestDocumentStats
from hawki_rag_text.chunking import split_text_into_chunks
from hawki_rag_text.markdown import (
...
def _ensure_tags(payload: dict[str, Any], text: str) -> None:
    """Preserve ingestion tag behavior using the leaf text package."""

    candidates = flatten_keywords(payload.get("tags"))
    for key in ("keywords", "labels"):
        if payload.get(key):
            candidates.extend(flatten_keywords(payload[key]))
    tags = normalize_tags(candidates) or fallback_tags(text)
...
def doc_job_id(default_job_id: str | None, doc: object) -> str | None:
    ...
    if isinstance(payload, dict):
        return (
            str(
                payload.get("job_id") or payload.get("trace_id") or default_job_id or ""
            )
            or None
        )
    ...
    *,
    ...
    for d in docs:
        ...
        if errors:
            ...
            doc_stats["validation_failures"].append(
                {"doc_id": source_doc_id, "errors": errors}
            )
            pipeline_log(
                logger,
                logging.WARNING,
        ...
        if warnings:
            ...
            pipeline_log(
                ...
                source_url=normalized_payload.get("source_url")
                or normalized_payload.get("page_url"),
            )
        ...
        chunks = split_text_into_chunks(document_text, chunk_chars, chunk_overlap) or [
            document_text
        ]
