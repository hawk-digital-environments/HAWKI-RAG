...
def record_embedding_failures(
    ...
    if fully_failed_doc_ids:
        doc_stats["skipped_docs"] = int(doc_stats.get("skipped_docs") or 0) + len(
            fully_failed_doc_ids
        )
        ...
        if isinstance(doc_ids, list):
            doc_stats["doc_ids"] = [
                doc_id for doc_id in doc_ids if str(doc_id) not in fully_failed_doc_ids
            ]
