...
def should_strip_converter_markdown_noise(payload: Mapping[str, Any] | None) -> bool:
    ...
    source_format = str(payload.get("source_format") or payload.get("format") or "").lower()
    if "markdown" in source_format:
        return True

    for key in ("relative_path", "markdown_path", "converted_path", "storage_path", "file_path"):
