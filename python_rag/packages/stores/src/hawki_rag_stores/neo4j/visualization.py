...
def _load_neo4j_driver_factory() -> Callable[..., Any]:
    neo4j_module = import_required_module(
        "neo4j",
        install_hint="Install hawki-rag-stores to export Neo4j graph visualizations.",
    )
...
class Neo4jGraphVisualization:
    ...
    def snapshot(
        self, *, limit: int | None = None, recent_doc_id: str | None = None
    ) -> dict[str, Any]:
        ...
        with self._session() as session:
            records = session.execute_read(
                lambda tx: list(tx.run(query, limit=effective_limit))
            )
        ...
        for record in records:
            ...
            relationship_doc_ids = [
                str(value) for value in (record.get("doc_ids") or []) if value
            ]
...
def write_graph_visualization(
    ...
    *,
    ...
    if (
        ...
        and database.strip()
        and database.strip() != (config.database or "")
    ):
