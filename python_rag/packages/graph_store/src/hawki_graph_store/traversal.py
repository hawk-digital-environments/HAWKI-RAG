...
from hawki_graph_store.errors import NEO4J_UNAVAILABLE_ERRORS
...
def _source_contains_label(source_norm: str, label: str) -> bool:
    ...
    if len(tokens) >= 2:
        return all(
            re.search(rf"\b{re.escape(token)}\b", source_norm) for token in tokens
        )
    ...
    *,
    ...
    for s, r, o in clean_triplets(triplets, graph_perf_log=graph_perf_log):
        if _is_known_prompt_example_triplet(s, o):
            ...
            continue
        if not (
            _source_contains_label(source_norm, s)
            or _source_contains_label(source_norm, o)
        ):
