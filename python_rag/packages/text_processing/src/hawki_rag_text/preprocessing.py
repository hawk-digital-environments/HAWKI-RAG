...
def _analyze_prompt_safety(text: str) -> dict[str, Any]:
    ...
    blocked = any(
        issue.startswith("prompt_injection") or issue.startswith("disallowed_token")
        for issue in issues
    )
    return {
...
def split_text(txt: str, target: int, overlap: int) -> list[str]:
    ...
    logger.debug(
        "split_text chunks=%s target=%s overlap=%s", len(chunks), target, overlap
    )
    return chunks
