"""Pure payload builders for the Qdrant vector-store adapter."""
...
def build_keyword_should_filter(
    ...
    *,
    ...
    clean_fields = [
        str(field).strip() for field in (fields or []) if str(field).strip()
    ]
...
def build_vector_search_body(
    vector: list[float], *, top_k: int, filter_body: dict[str, Any]
) -> dict[str, Any]:
    return {
        "vector": vector,
        "limit": int(top_k),
