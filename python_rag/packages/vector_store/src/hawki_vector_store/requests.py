"""Typed requests owned by the Qdrant vector-store adapter."""
...
def build_count_points_request(
    collection: str, *, exact: bool, timeout: float
) -> QdrantRequest:
    return build_count_points_request_with_filter(
        collection, exact=exact, timeout=timeout
    )
