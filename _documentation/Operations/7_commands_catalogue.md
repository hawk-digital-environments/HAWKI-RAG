...
| `docker exec -it hawki_rag_app php artisan pipeline:deduplication-backfill` | Audit historical managed uploads without writing. Verification requires a matching server-owned raw-upload checksum, current dataset/output scope, and matching Qdrant output points; unverified page-registry records remain lazy and process once. |
| `docker exec -it hawki_rag_app php artisan pipeline:deduplication-backfill --apply` | Idempotently seed only the verified historical deduplication states. Existing states are always preserved. Run the dry-run form first and review its conflict/deferred counts. |
