...
2) Set strong secrets in `.env` (APP_KEY, DB/RabbitMQ passwords).
3) Keep only Nginx port 8080 public; do NOT expose DB, Qdrant, Neo4j, RabbitMQ.
4) (Optional) Change MariaDB binding to localhost-only or internal network only.
5) Run: `make network && make up-core && make up-rag`.
...
- Queues: use RabbitMQ for heavier async workloads; add more workers.
