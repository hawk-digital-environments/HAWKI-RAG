...
| `is_optimized=true` and not fast | Higher-recall HNSW search with threshold 0.28 and optional preferred tags; empty results remove the threshold |
...
A **separate** related-graph read collects terms from the query and final hits,
using at most 30 terms by default. This produces `kg`, independently of the
...
separate fact read. Up to 20 facts are appended to the generation prompt.
...
Laravel's current query scope always enables graph access when it can construct
the scope. That does not prove graph ingestion ran or facts exist.
